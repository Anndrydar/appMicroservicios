...
export class PartidosService {
  ...
  async update(id: string, updatePartidoDto: UpdatePartidoDto): Promise<Partido> { 
    return this.partidoModel.findByIdAndUpdate({ _id: id }, updatePartidoDto, { 
      new: true, 
    });
  }
  ...
  async getVer(request: Request) {
    return await this.partidoModel.find(request.query).populate('idEquipo2')
    .setOptions({ sanitizeFilter: true })
    .exec();
  }
}
